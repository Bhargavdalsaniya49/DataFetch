...
const UserTable = () => {
  ...
  const handleSearch = () => {
    if (searchQuery.trim()) {
      ...
      setFilteredUsers(userData); // Show all users if search query is empty
    }
  };
          ...
          {filteredUsers.length > 0 ? (
            filteredUsers.map((user) => (
              <tr key={user.id}>
                ...
                <td>
                  ...
                  City: {user.company.address.city} <br />
                  State: {user.company.address.state} <br />
                  State Code: {user.company.address.stateCode} <br />
                  Postal Code: {user.company.address.postalCode} <br />
                </td>
                <td>
                  {user.company.address.coordinates ? (
                    <>
            ...
            <tr>
              <td colSpan="38" style={{ textAlign: "center" }}>
                {searchQuery ? `No user found with search term: "${searchQuery}"` : "No users available"}
              </td>
            </tr>
          )}
        </tbody>
